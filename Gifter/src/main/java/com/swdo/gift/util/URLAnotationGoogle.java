...
public class URLAnotationGoogle {
	...
	public static void AsyncBatchAnnotateImages(ArrayList<String> imgURL) {
		...
		ArrayList<AnnotateImageRequest> requestList = new ArrayList<AnnotateImageRequest>();
	      try (ImageAnnotatorClient imageAnnotatorClient = ImageAnnotatorClient.create()) {
	    	...
	    	for(int i = 0; i < 30; i++) {
	    		//ImageSource source = ImageSource.newBuilder().setImageUri(imgURL.get(i)).build();4
	        ImageSource source = ImageSource.newBuilder().setImageUri(inputImageUri).build();
		        ...
		        requestList.add(requestsElement);
	    	}
